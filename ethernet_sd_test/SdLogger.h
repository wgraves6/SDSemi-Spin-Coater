/*
  ...
  Pulled out of ethernet_sd_test.ino once basic read/write/list was verified
  on hardware, so it can be dropped into improved_spin_coater/ later for
  telemetry logging.

  Integration notes for improved_spin_coater:
  - Call begin() once during setup(), after any other SPI device on the bus
    has been constructed but before it's used - it drives both CS pins HIGH
    before touching SD.begin(), which is required because the SD slot and
    the shield's W5100 chip share MOSI/MISO/SCK.
  - appendLine()/printFile() call SD.open()/close(), which can block for a
    ...
    path - call them from the same once-a-second telemetry cadence the MQTT
    publish already uses, not from the ISR or the PID/motor update code.
